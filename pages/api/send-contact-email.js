...
export default async function handler(req, res) { // this is not being used
  if (req.method === 'POST') {
    const { to, subject, firstName, enquiryNumber, question, phoneNum} = req.body;

    // Read and compile the HTML template
    const templatePath = path.resolve('./src/htmlEmailTemplates/mainCustomerTemplate.html');
    const templateSource = fs.readFileSync(templatePath, 'utf8');
    const template = Handlebars.compile(templateSource);

    // Prepare the HTML with dynamic data
    const htmlContent = template({ firstName, enquiryNumber, question, phoneNum});
